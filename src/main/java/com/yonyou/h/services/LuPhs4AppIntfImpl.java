...
public class LuPhs4AppIntfImpl implements LuPhs4AppIntf {
	...
	private static final String[] YlfdNames = { "城镇职工基本医疗保险", "城镇居民基本医疗保险", "新型农村合作医疗", "贫困救助", "商业医疗保险", "综合医疗保险",
			"住院医疗保险", "农民工医疗保险", "少儿住院及门诊医保", "全自费", "全公费", "其他" };
	...
	private void saveGrda(DBAgent dba, PoDaDaGrda0 grda0, PoDaDaGrda1 grda1, PoDaDaGrda2 grda2) {
		WsxxBa wsxxBa = null;
		...
		wsxxBa = ArchivemgrHelper.PoDaDaGrda0ToWsxxBa(wsxxBa, grda0, grda1.getHklb());// 转换病案信息
		if (grda1 != null && "0".equals(grda1.getHzgx()) && (grda0.getJtdaid() == null)) { // 如果当前人员选择为户主时建立家庭档案
		...
		try {
			...
			if (da0List.size() == 0) {
				...
				if (!da0.getState().equals("1")) {
					getDBAgent().executeSQL("UPDATE DA_GRDA0 SET STATE=1 WHERE EMPI=:EMPI", condi);
				}
			}
